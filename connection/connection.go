...
func (s *impl) Acknowledge(p *mqttp.ConnAck, opts ...Option) error {
	...
	err := s.sess.SetReadDeadline(time.Time{})
	if err != nil {
		return err
	}
	...
	if _, err = bufs.WriteTo(s.sess); err != nil {
		ack = ErrConnectionNack
	} else {
...
func (s *impl) onConnectionCloseStage1(error) {
	...
	_ = s.sess.SetReadDeadline(time.Time{})
	select {
	case <-s.connect:
	default:
		close(s.connect)
...
func (s *impl) onConnectionCloseStage2(status error) {
	...
	s.onStop.Do(func() {
		...
		_ = s.sess.SetReadDeadline(time.Now().Add(time.Microsecond))
		s.rx.shutdown()
	...
	if reason, ok := status.(mqttp.ReasonCode); ok &&
		...
		if buf, err = mqttp.Encode(pkt); err != nil {
			...
			if written, err = s.sess.Write(buf); written != len(buf) {
